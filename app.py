...
async def run_qalia_analysis(repo_url: str, branch: str = "main", repo_path: str = None, installation_id: int = None) -> Dict[str, Any]:
...
async def handle_pull_request(payload: Dict[str, Any]):
    ...
    try:
        ...
        commit_message = payload.get("pull_request", {}).get("head", {}).get("commit", {}).get("message", "")
        
        # Skip if this commit was made by Qalia
        if is_qalia_commit(payload):
            logger.info("🔄 Skipping analysis - this is a Qalia-generated commit (infinite loop prevention)")
            return
...
*Powered by Qalia.ai - Real-time progress updates*
        ...
        try:
            ...
            analysis_results = await asyncio.wait_for(
                run_qalia_analysis(repo_url, branch, repo_path, installation_id),
...
async def handle_push(payload: Dict[str, Any]):
    ...
    # INFINITE LOOP PREVENTION: Check if this is a Qalia-generated commit
    if is_qalia_commit(payload):
        logger.info("🔄 Skipping push analysis - this is a Qalia-generated commit (infinite loop prevention)")
        return
    ...
    try:
        # Run QA AI analysis
        analysis_results = await run_qalia_analysis(repo_url, branch, repo_path, installation_id)
    finally:
        # Clean up cloned repository
        if repo_path:
            shutil.rmtree(repo_path, ignore_errors=True)
...
def is_qalia_commit(payload: Dict[str, Any]) -> bool:
    """
